const _ = require('lodash');
const async = require('async');
const keystone = require('keystone');
const passport = require('passport');
const passportGitHubStrategy = require('passport-github').Strategy;
const request = require('request');
const User = keystone.list('User');
const applicationLogger = require('timestamp-logger');

const credentials = {
	clientID: process.env.GITHUB_CLIENT_ID,
	clientSecret: process.env.GITHUB_CLIENT_SECRET,
	callbackURL: process.env.GITHUB_CALLBACK_URL
};

exports.authenticateUser = function (req, res, next) {
	const self = this;

	const redirect = '/auth/confirm';

	// Begin process
	applicationLogger('[services.github] - Triggered authentication process...');

	// Initialise GitHub credentials
	const gitHubStrategy = new passportGitHubStrategy(credentials, function (accessToken, refreshToken, profile, done) {
		done(null, {
			accessToken: accessToken,
			refreshToken: refreshToken,
			profile: profile
		});
	});

	// Pass through authentication to passport
	passport.use(gitHubStrategy);

	// Save user data once returning from GitHub
	if (_.has(req.query, 'cb')) {

		applicationLogger('[services.github] - Callback workflow detected, attempting to process data...');

		passport.authenticate('github', {session: false}, function (err, data) {

			if (err || !data) {
				applicationLogger(`[services.github] - Error retrieving GitHub account data - ${JSON.stringify(err)}`);
				return res.redirect('/sign-in');
			}

			applicationLogger('[services.github] - Successfully retrieved GitHub account data, processing...');

			const name = data.profile && data.profile.displayName ? data.profile.displayName.split(' ') : [];

			const auth = {
				type: 'github',

				name: {
					first: name.length ? name[0] : '',
					last: name.length > 1 ? name[1] : ''
				},

				website: data.profile._json.blog,

				profileId: data.profile.id,

				username: data.profile.username,
				avatar: data.profile._json.avatar_url,

				accessToken: data.accessToken,
				refreshToken: data.refreshToken
			};

			// GitHub Specific: Retrieve email address
			self.getEmails(auth.accessToken, function (err, email) {
				if (!err && email) auth.email = email;
				req.session.auth = auth;
				return res.redirect(redirect);
			});

		})(req, res, next);

		// Perform initial authentication request to GitHub
	} else {

		applicationLogger('[services.github] - Authentication workflow detected, attempting to request access...');

		passport.authenticate('github', {scope: ['user:email']})(req, res, next);

	}

};

exports.getEmails = function (accessToken, next) {
	applicationLogger('[services.github] - Finding GitHub email addresses...');

	request({
		url: `https://api.github.com/user/emails?access_token=${accessToken}`,
		headers: {
			'User-Agent': 'keystonejs-passportjs-examples.herokuapp.com'
		}
	}, function (err, data) {

		if (err) {

			applicationLogger(err);
			applicationLogger('[services.github] - Error retrieving GitHub email addresses.');

			return next(err);

		} else {

			applicationLogger('[services.github] - Retrieved GitHub email addresses...');

			const emails = JSON.parse(data.body);
			let primaryEmail = false;

			if (emails.length) {
				_.each(emails, function (e) {
					if (!e.primary) return;
					primaryEmail = e.email;
				});
			}

			return next(err, primaryEmail);

		}

	});
};
